...
function launchImagingPrint(o: OrderSummary, t: (k: string, d: string) => string) {
  ...
  const dispose = showModal('path-drc-print-report-modal', {
    reportKind: capitalize(t('imaging', 'imaging')),
    ...
    procedureLabel: t('radiologyImaging', 'Radiology / Imaging'),
...
const OrdersTab: React.FC<{ orders: OrderSummary[]; isLoading: boolean; onRefresh: () => void; patientUuid: string }> = ({
  orders,
  isLoading,
  onRefresh,
  patientUuid,
  ...
  const addButton = (
    <Button
      ...
      onClick={() => launchWorkspace2('add-imaging-order', { patientUuid, kind: 'imaging' })}>
  ...
  return (
    <div style={{ backgroundColor: '#fff', padding: '1rem' }}>
      <SectionHeading title={t('radiologyImagingOrders', 'Radiology & Imaging Orders')} action={addButton} />
      {isLoading ? (
        <DataTableSkeleton headers={headers} showHeader={false} showToolbar={false} />
      ) : rows.length === 0 ? (
        <p style={{ color: '#525252', padding: '1rem 0' }}>
          {t('noImagingOrdersToDisplay', 'There are no imaging orders to display')}
        </p>
      ) : (
...
const ResultsTab: React.FC<{
  ...
  onRefresh: () => void;
  patientUuid: string;
}> = ({ orders, isLoading, onRefresh, patientUuid }) => {
  const { t } = useTranslation();
  ...
  return (
    <div style={{ backgroundColor: '#fff', padding: '1rem' }}>
      <SectionHeading title={t('imagingResults', 'Imaging Results')} onRefresh={onRefresh} />
      {isLoading ? (
        <DataTableSkeleton headers={headers} showHeader={false} showToolbar={false} />
      ) : rows.length === 0 ? (
                          ...
                          <TableExpandedRow colSpan={hdrs.length + 1}>
                            <div style={{ padding: '0.5rem 0' }}>
                              ...
                              <div style={{ display: 'flex', justifyContent: 'flex-end', paddingTop: '0.75rem' }}>
                                <Button kind="tertiary" size="sm" onClick={() => launchImagingPrint(order, t)}>
...
const ImagingPatientDashboard: React.FC<ImagingPatientDashboardProps> = ({ patientUuid }) => {
  ...
  const orderTypeUuid = config.imaging?.orderTypeUuid;
  const conceptSetUuid = config.imaging?.conceptSetUuid;
  ...
  // Scope to the imaging concept set (imaging and procedure share an OrderType).
      ...
      <Tabs>
        ...
        <TabPanels>
          <TabPanel style={{ padding: 0 }}>
            <OrdersTab orders={imagingOrders} isLoading={isLoading} onRefresh={() => mutate()} patientUuid={patientUuid} />
          </TabPanel>
          <TabPanel style={{ padding: 0 }}>
            <ResultsTab
